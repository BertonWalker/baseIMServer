...
class DB {
    ...
    async increaseMaxMsgVersion(username, increaseId = 1) {
        if(!username) {
            return false;
        }
        const {acknowledged} = await this.db.collection('userInfo').updateOne({username}, {$inc: {maxMsgId: increaseId}}, { upsert: true })
        if (!acknowledged) {
            logger.info('update fail', acknowledged);
            return Promise.reject(new Error('increase MaxMsgVersion fail'));
        }
        return Promise.resolve();
    ...
    async queryFriend() {
        return this.db.collection('userInfo').find().toArray()
    }
